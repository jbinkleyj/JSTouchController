ShipMoving = (x,y) ->
  pos = @pos = new Vector2(x,y)
  @angle = 0
  vel = @vel = new Vector2(0,0)
  targetVel = @targetVel = new Vector2(0,0)
  temp = new Vector2(0,0)
  thrustSize = 0
  canvas = @canvas = document.createElement("canvas")
  canvas.width = 60
  canvas.height = 60
  canvas.style = "display:block; position:absolute; background-color:'#ff0000';"
  canvas.style.webkitTransformOrigin = canvas.style.MozTransformOrigin = canvas.style.OTransformOrigin = canvas.style.transformOrigin = "30px 30px"
  c = canvas.getContext('2d')
  @c = c
  counter = 0

  this.update = ->
    # speed limit
    maxSpeed = 30
    if targetVel.isMagGreaterThan(maxSpeed)
      targetVel.normalise()
      targetVel.multiplyEq(maxSpeed)

    unless targetVel.equals(vel)
      temp.copyFrom(targetVel)
      temp.minusEq(vel)
      temp.multiplyEq(0.3) if temp.isMagGreaterThan(0.001)
      vel.plusEq(temp)

    pos.plusEq(vel)
    @angle = vel.angle() if vel.isMagGreaterThan(0)

    thrustSize = vel.magnitude()

  this.draw = ->
    c.clearRect(0,0,60,60)
    c.fillStyle = "rgba(255,255,255,0.5)"
    c.save()
    c.translate(30,30)
    c.strokeStyle = "#fff"
    c.lineWidth = 2

    c.beginPath()
    c.moveTo(-10, -10)
    c.lineTo(-10, 10)
    c.lineTo(14, 0)
    c.closePath()
    c.stroke()

    if thrustSize > 0
      c.beginPath()
      c.moveTo(-10, -6)
      c.lineTo(-10 - (thrustSize/((counter%2)+1)) , 0)
      c.lineTo(-10, 6)
      c.stroke()
      counter++

    c.restore()

    posx = Math.round(pos.x - 30)
    posy = Math.round(pos.y - 30)

    styleStr = "translate3d("+posx+"px, "+posy+"px, 0px) rotate("+@angle+"deg)"
    canvas.style.webkitTransform = canvas.style.MozTransform = canvas.style.OTransform = canvas.style.transform = styleStr
    console.log(styleStr)
    
  return this
