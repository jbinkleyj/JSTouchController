...
ShipMoving = function(x, y) {
  var c, canvas, counter, pos, targetVel, temp, thrustSize, vel;
  pos = this.pos = new Vector2(x, y);
  this.angle = 0;
  vel = this.vel = new Vector2(0, 0);
  targetVel = this.targetVel = new Vector2(0, 0);
  ...
  this.update = function() {
    ...
    return thrustSize = vel.magnitude();
  };
  this.draw = function() {
    var posx, posy, styleStr;
    ...
    c.restore();
    posx = Math.round(pos.x - 30);
    posy = Math.round(pos.y - 30);
    styleStr = "translate3d(" + posx + "px, " + posy + "px, 0px) rotate(" + this.angle + "deg)";
    canvas.style.webkitTransform = canvas.style.MozTransform = canvas.style.OTransform = canvas.style.transform = styleStr;
    return console.log(styleStr);
  };
  return this;
};